...
export default function Orders() {
  ...
  const handlePrintOrder = (order: any) => {
    ...
    toast({
      title: 'Imprimiendo',
      description: `Pedido #${order.display_number} enviado a impresora`,
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Pedido #${order.display_number} movido a ${columns.find(c => c.status === newStatus)?.title}`,
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 dark:bg-white/10 px-3 py-0.5 tabular-nums">
              ...
              <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">
                {formatInTimeZone(now, storeTimezone, 'HH:mm aaaa')}
              </span>
            </span>
          </div>
        </div>
